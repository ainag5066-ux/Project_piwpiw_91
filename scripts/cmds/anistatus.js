        ...
        if (puti.data.videoUrl) {
          ...
          if (fs.existsSync(cacheFilePath)) {
            await api.sendMessage({
              body: "Piw Piw Chat Bot Random anime status video.",
              attachment: fs.createReadStream(cacheFilePath),
            }, threadID, event.messageID);

            fs.unlinkSync(cacheFilePath);
          } else {
            api.sendMessage("Piw Piw Chat Bot Error downloading the video.", threadID);
          }
        } else {
          api.sendMessage("Piw Piw Chat Bot Error fetching video URL.", threadID);
        }
      } else {
        api.sendMessage("Piw Piw Chat Bot Error fetching data from external API.", threadID);
      }
    } catch (err) {
      console.error(err);
